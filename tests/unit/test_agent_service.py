...
class TestMessengerAgentService:
    ...
    def mock_settings(self):
        ...
        with patch("src.services.agent_service.get_settings") as mock:
            ...
            settings.default_model = "gateway/anthropic:claude-3-5-sonnet-latest"
            settings.fallback_model = "gateway/anthropic:claude-3-5-haiku-latest"
    ...
    async def test_respond_returns_agent_response(self, agent_context, mock_settings):
        ...
        with patch("src.services.agent_service.Agent") as MockAgent:
            ...
            response = await service.respond(
                agent_context, "What services do you offer?"
            )
    ...
    @pytest.mark.asyncio
    async def test_respond_handles_errors_gracefully(
        self, agent_context, mock_settings
    ):
    ...
    def test_agent_response_should_escalate(self):
        ...
        response1 = AgentResponse(
            message="Test", confidence=0.9, requires_escalation=False
        )
        assert response1.should_escalate(threshold=0.7) is False

        # Low confidence
        response2 = AgentResponse(
            message="Test", confidence=0.5, requires_escalation=False
        )
        assert response2.should_escalate(threshold=0.7) is True

        # Escalation flag set
        response3 = AgentResponse(
            message="Test", confidence=0.9, requires_escalation=True
        )
    ...
    async def test_respond_with_fallback(self, agent_context, mock_settings):
        ...
        with (
            patch("src.services.agent_service.Agent") as MockAgent,
            ...
            service = MessengerAgentService()
            response = await service.respond_with_fallback(
                agent_context, "Test message"
            )
    ...
    @pytest.mark.asyncio
    async def test_system_prompt_includes_reference_doc(
        self, agent_context, mock_settings
    ):

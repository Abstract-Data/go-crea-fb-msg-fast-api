...
class AgentResponse(BaseModel):
    ...
    confidence: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Confidence score from 0.0 to 1.0"
    )
    requires_escalation: bool = Field(
        default=False, description="Whether this should be escalated to a human"
    )
    escalation_reason: str | None = Field(
        default=None, description="Reason for escalation if requires_escalation is True"
    )
